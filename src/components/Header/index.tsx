...
const Header = () => {
  const { logo, siteTitle } = useSiteMetadata();
  ...
  return (
    <nav className="mx-auto mt-12 flex w-full max-w-screen-2xl items-center justify-between pl-6 lg:px-16">
      <div className="flex items-center gap-3">
        <picture>
          <img className="h-12 w-12 rounded-full" alt="logo" src={logo} />
        </picture>
        <span className="text-lg font-semibold">{siteTitle}</span>
      </div>
      <div className="flex items-center pr-6 lg:pr-0">
        <button
          type="button"
